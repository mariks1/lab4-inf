...
def parsing(txt, cnt):  
    ...
    while True:                                 
        ...
        if stringWbracket.match(txt[cnt]) is not None:
            ...
            first,second = (string.match(txt[cnt])).groups()
            data[first] = second
            cnt+=1
    cnt+=1
    return data, cnt

lines = open("file.json", "r",encoding='utf-8').read().splitlines()
...
def file_write(string):
    with open('cringe2.xml', 'a',encoding='utf-8') as the_file:
